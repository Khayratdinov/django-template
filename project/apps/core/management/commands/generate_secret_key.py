...
from project.apps.core.management.utils import generate_secret_key, save_to_settings
...
class Command(BaseCommand):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument(
            ...
            "--print",
            action="store_true",
            help="Display the generated secret key " "while saving on file.",
        )
        parser.add_argument(
            "-d",
            "--settings-dir-name",
    ...
    def handle(self, *args, **kwargs):
        ...
        if kwargs["save"]:
            ...
            try:
                save_to_settings(
                    f"'{parameter_value}'", parameter_name, settings_dir_name
                )
                self.stdout.write(
                    self.style.SUCCESS(f"{parameter_name} updated successfully.")
                )
                if kwargs["print"]:
                    self.stdout.write(
                        "{} {}".format(
