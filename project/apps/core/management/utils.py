...
def generate_secret_key(length=50):
    ...
    if not isinstance(length, int):
        raise TypeError(f"invalid literal for int() with base 10: {length}")
...
def save_to_settings(value, parameter_name, settings_dir_name=""):
    ...
    settings_dir = os.path.join(settings.PROJECT_DIR, settings_dir_name, "base.py")
    if not os.path.exists(settings_dir):
        raise FileNotFoundError("Can't find `base.py` file: " f"{settings_dir}")
...
def _replace_line(value, parameter_name, settings_file):
    ...
    if parameter_name:
        ...
        line_pattern = rf"^{parameter_name} = .*"
